...
const Register = () => {
  ...
  // Ép cuộn lên đầu trang khi vừa vào
  useEffect(() => {
  ...
  const handleRegister = async (e) => {
    ...
    if (password !== confirmPassword) {
        const errMsg = 'Mật khẩu xác nhận không khớp! Vui lòng kiểm tra lại.';
        setError(errMsg);
        showToast(errMsg, 'error'); // Bắn Toast lỗi
        return;
    }
    ...
    try {
      ...
      if (response.ok) {
        // 🔥 Đã thay alert() bằng showToast xịn sò
        showToast('Tạo tài khoản thành công! Hãy đăng nhập để bắt đầu.', 'success');
        navigate('/login');
      } else {
        const errorText = await response.text();
        const errMsg = errorText || 'Tài khoản hoặc Email đã tồn tại!';
        setError(errMsg);
        showToast(errMsg, 'error'); // Bắn Toast lỗi từ Backend
      }
    } catch (err) {
      const errMsg = 'Lỗi kết nối đến Server! Kiểm tra lại mạng hoặc Backend.';
      setError(errMsg);
      showToast(errMsg, 'error'); // Bắn Toast lỗi sập server
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <form onSubmit={handleRegister} className="space-y-4 relative z-10">
          <div>
            ...
            <input 
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors"
          ...
          <div>
            ...
            <div className="relative">
              <input 
                ...
                className="w-full px-4 py-2.5 pr-12 rounded-xl bg-slate-900 border border-slate-700 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors"
                placeholder="••••••••" 
              />
              <button 
                type="button" 
